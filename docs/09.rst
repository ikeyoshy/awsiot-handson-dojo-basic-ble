...
ハンズオン終了後のAWSの課金発生を抑えるために、今回のハンズオンで作成したAWSのリソースを削除します。BLEゲートウェイの電源を切れば、AWS IoTでの課金は発生しなくなりますので、後日、削除して頂いても大丈夫です。S3とElastic Search Serviceだは、入力データが無くても課金対象になりますので、必ず削除して下さい。
