...
2. Wi-Fiでインターネットに接続できる環境
   ...
   * Guest WiFiの様なパスワード(WPA/WPA2-PSK方式)で接続可能なWiFi環境が必要です。

3. AWSアカウント

   * 基本的に参加者お一人様につき、個別のAWSアカウント持って頂くことが前提です。
   * 複数の参加者で1つのAWSアカウントを共用してハンズオンを実施することも可能です。その場合。事前にIAMユーザーを作って頂くことになります。IAMユーザーの作成方法は、事前準備の項を参照して下さい。
...
ルートユーザーの方は、IAMユーザーで参加される方に、下記の連絡をお願い致します。これらは、ダウンロードしたcsvファイルに記載されています。
...
※ アクセスキー ID、シークレットアクセスキーは、可視化のシナリオで使用します。その際、保存したcsvファイルを参照して下さい。
