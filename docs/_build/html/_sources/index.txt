...
* PC、もしくは Mac
  ...
  - 各種設定を行うためにWebブラウザを使用します。インターネットエクスプローラーとFireFoxでは、問題が出る場合が有りますので、Webブラウザは、必ず、Google Chromeをお使い下さい。
* ACコンセント
...
複数の参加者で1つのAWSアカウントを共用する場合は、他の参加者のためにIAMユーザーを作成して頂く必要があります。ルートアカウントを使用する場合でも、ハンズオン内でグラフ表示を行うときにアクセスキーID、シークレットアクセスキーを使用しますので、IAMユーザーを策せ死して下さい。
